...
void write_packet_csv(const std::string &filename, int seq, double start, double end) {
    static std::ofstream csv("trace/" + filename + ".csv", std::ios::app);
    if (seq == 1) {
        csv << "start\tend\tCellId\tIMSI\tRNTI\tLCID\tnTxPDUs\tTxBytes\tnRxPDUs\tRxBytes\tdelay\tstdDev\tmin\tmax\tPduSize\tstdDev\tmin\tmax\n";
    }

    int tx = 1000, rx = 1000, pdu_size = 544;
    double delay = 0.000724998;
    int cellId = 1, imsi = 1, rnti = 1, lcid = 3;
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        write_packet_csv(out_file, i + 1, i * 0.25, (i + 1) * 0.25);
        sleep(1);
    }
    ...
    std::cout << "Trace and stats saved.\n";
    return 0;
}
